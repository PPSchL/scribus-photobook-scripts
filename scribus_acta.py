...
def set_g_pos_moisjour(page,g_pos):
    ...
    def invert4rightpage(label,g_pos,page):
        ...
        g_pos[label].update({"dx": page.xs-page.mright-page.mleft-g_pos[label]["xs"]-g_pos[label]["dx"], "dy": g_pos[label]["dy"]})
        return g_pos
...
def set_g_pos_variable(group_type,g_pos,page,n_groups,gutter):
    if group_type=="normal":
        ...
        g_pos["Acta_img"]={"xs":page.xs-page.mleft-page.mright-g_pos["Acta_txt"]["xs"]}
        #image and text height are identical and determnined by number of groups and gutter
        g_pos["Acta_img"]["ys"]=sp.pict_size1D(n_groups, page.mtop, page.mbottom, gutter, page.ys)
        ...
        if group_type=="central":
            ...
            g_pos["Acta_img"]["ys"] = sp.pict_size1D(n_groups,page.mtop,page.mbottom,gutter,page.ys)-g_pos["Acta_txt"]["ys"]
            ...
            g_pos["Acta_img"]["ys"] = 2*sp.pict_size1D(n_groups,page.mtop,page.mbottom,gutter,page.ys)+ gutter-g_pos["Acta_txt"]["ys"]
        else:#group_type=="whole_page"
            # increase text heigh to 4 lines
            g_pos["Acta_txt"]["ys"] = 27.0
            # image height equals whole available page size-text height determined above
            g_pos["Acta_img"]["ys"] = page.ys-page.mtop-page.mbottom-g_pos["Acta_txt"]["ys"]
        #calculate text and image position relative to topleft corner coordinates
...
def paste_and_resize_group(group_type,page,n_groups,group_n,g_pos,top_group,gutter):
    ...
    y_topleft=sp.pict_pos1D(group_n, page.mtop, sp.pict_size1D(n_groups,page.mtop,page.mbottom,gutter,page.ys), gutter)
    ...
    if group_type!="normal":
        ...
        for elem in current_g:
            if elem.find("Acta_img") != -1:
                elem_obj=sp.set_object_info(elem,x_topleft+g_pos["Acta_img"]["dx"],y_topleft+g_pos["Acta_img"]["dy"],g_pos["Acta_img"]["xs"],g_pos["Acta_img"]["ys"],0,0,0,0,0)
                sp.split_image("resize", elem_obj, x_n_picts, y_n_picts, gutter)
    
def draw_1_group(group_type,page,path_to_base,n_groups,group_n,gutter,top_group,below_groups,g_pos):
    copy_group(path_to_base,group_n, top_group,below_groups)
    paste_and_resize_group(group_type,page,n_groups,group_n,g_pos,top_group,gutter)

def draw_normal_page(page,path_to_base,n_groups,gutter,top_group,below_groups,g_pos):
    for group_n in range(1,n_groups+1):
        draw_1_group("normal",page,path_to_base,n_groups,group_n,gutter,top_group,below_groups,g_pos)
